...
import {UserType, type Users} from '../graphql/schema.js'
...
export const Mutation = new GraphQLObjectType({
    ...
    fields: {
        createUser: {
            ...
            //     return newUser;
            // }
            async resolve(parent: unknown, args: Partial<Users>, context: any): Promise<Users>{
                const conn : Pool = context?.pool;
                if (!conn) {
        ...
        UpdateUser: {
            ...
            async resolve(parent: unknown, args: Partial<Users>, context: any): Promise<Users>{
                ...
                try{
                    ...
                    const [update_rows] = await conn.query<RowDataPacket[]>(
        ...
        deleteUser: {
            ...
            async resolve(parent:unknown, args: {id?: number}, context:any): Promise<Users>{
                const conn : Pool = context?.pool;
                if (!conn) {
                    throw new Error("Database connection not found in context");
                }
                try{
                    ...
                    const [delete_rows] = await conn.query<RowDataPacket[]>(
                        'SELECT * FROM users WHERE id = ? ',
                        [args.id]
                    )
                    // 2. delete content
