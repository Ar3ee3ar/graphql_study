...
export const RootQuery = new GraphQLObjectType({
    ...
    fields: {
        getAllUsers:{ // query command
            ...
            // },
            async resolve(parent: unknown, args:any, context: any): Promise<Users[]>{
                ...
                try{
                    ...
                    return rows.map((row: any) => mapUser(row));
        ...
        getUser: {
            ...
            // },
            async resolve(parent: unknown, args: {id?: number}, context: any): Promise<Users | null>{
                const conn : Pool = context?.pool;
                if (!conn) {
                    throw new Error("Database connection not found in context");
                }
